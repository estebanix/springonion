...
export default function BlogContent(props){
    ...
    const blogs = blogData.map(dat => {
        ...
        return <div className="big-blog--container" key={dat.id}>
            <div className="big-blog--container-img" style={{backgroundImage:`url(${require(`../Images/aid-data-images/${dat.coverImg}`)})`}}></div>
            <div className="big-blog--container-content">
                <h3>{dat.title}</h3>
                <div className="overflow--blog-text">
                    <p>{dat.content}</p>
                </div>
                <Link to="/currentBlog"><button 
                onClick={() => handleButtonClick(dat)} 
                className="big-blog-more-btn"
                >
